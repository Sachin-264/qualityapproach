...
// RENAMED METHOD: Previously fetchDataFromFullUrl or fetchGenericData. Now specifically for report data format.
// This method processes data that is expected to be a List of Map<String, dynamic>
...
const Duration(seconds: 30), // Increased timeout for potentially larger data
onTimeout: () {
...
return _parseApiResponse(response); // Use the existing parser for report data
} catch (e) {
...
// NEW METHOD: For fetching raw JSON data from any URL, without assuming a specific structure (e.g., for PrintPreview)
...
return {}; // Return empty map or null, depending on expected empty state
}
return jsonDecode(response.body); // Directly decode the body, return as dynamic
} else {
...
// Ensure 'is_api_driven' and 'api_url' are correctly parsed
fieldItem['is_api_driven'] = item['is_api_driven'] == '1' || item['is_api_driven'] == true;
fieldItem['api_url'] = item['api_url']?.toString() ?? '';
// Parse JSON strings for field_params and payload_structure
...
// MODIFIED: Added is_api_driven and api_url and user_filling fields to the payload
...
// NEW: Add API driven and user filling fields
'is_api_driven': field['is_api_driven'] == true ? 1 : 0,
'api_url': field['api_url']?.toString() ?? '',
'field_params': jsonEncode(field['field_params'] ?? []), // Store as JSON string
'is_user_filling': field['is_user_filling'] == true ? 1 : 0,
'updated_url': field['updated_url']?.toString() ?? '',
'payload_structure': jsonEncode(field['payload_structure'] ?? []), // Store as JSON string
};

debugPrint('Saving field config for ${field['Field_name']}: is_api_driven=${payload['is_api_driven']}, api_url=${payload['api_url']}, field_params=${payload['field_params']}, is_user_filling=${payload['is_user_filling']}, updated_url=${payload['updated_url']}, payload_structure=${payload['payload_structure']}');
...
// MODIFIED: Passed Report_name, Report_label, API_name directly to Demo_table payload
...
// NEW METHOD: deployReportToClient
...
// NEW METHOD: postJson for generic POST requests with JSON payload
