...
// Use display_value_cache if available, otherwise fall back to value
String textToDisplayInField = param['display_value_cache']?.toString() ?? param['value']?.toString() ?? '';
...
// After modal returns, update UI text field to show the display label or API value
String finalValueForUI = result['display_label']?.toString() ?? result['value']?.toString() ?? '';
paramController.text = finalValueForUI;
paramController.selection = TextSelection.fromPosition(TextPosition(offset: paramController.text.length));
debugPrint('  UI controller text updated to: "$finalValueForUI" (from modal result)');
