...
class EditReportPage extends StatefulWidget {
  ...
  const EditReportPage({Key? key, required this.reportName, required this.recNo}) : super(key: key);
...
class _EditReportPageState extends State<EditReportPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => EditReportBloc()..add(FetchEditReportData(widget.userCode, widget.companyCode, widget.recNo)),
        child: Padding(
                    ...
                    child: BlocConsumer<EditReportBloc, EditReportState>(
                      listener: (context, state) {
                        ...
                        if (state.columns.isNotEmpty && _nameControllers.isEmpty) {
                          _originalColumnNames = state.columns.map((column) => column.columnName).toList();
                          _nameControllers = state.columns
                              .map((column) => TextEditingController(
                            text: column.columnHeading.isEmpty ? column.columnName : column.columnHeading,
                          ))
                              .toList();
                        }
                      },
                                          ...
                                          cells: [
                                            ...
                                            DataCell(
                                              Container(
                                                ...
                                                child: TextField(
                                                  ...
                                                  decoration: InputDecoration(
                                                    hintText: _originalColumnNames[index], // Use column name as hint
                                                  ...
                                                  onChanged: (value) {
                                                    context.read<EditReportBloc>().add(UpdateEditColumnName(index, value));
                                                  },
                                                ),
                                              ),
                                            ),
                                            DataCell(
                                              Checkbox(
                                                ...
                                                onChanged: (value) {
                                                  context.read<EditReportBloc>().add(UpdateEditColumnVisibility(index, value!));
                                                },
                              ...
                              children: [
                                ElevatedButton.icon(
                                  onPressed: () async {
                                    ...
                                    context.read<EditReportBloc>().add(SubmitEditReport(reportName, widget.userCode, widget.companyCode, widget.recNo));
                                    ScaffoldMessenger.of(context).showSnackBar(
                                ...
                                ElevatedButton.icon(
                                  onPressed: () {
                                    ...
                                    context.read<EditReportBloc>().add(ResetEditReport());
                                    for (var i = 0; i < _nameControllers.length; i++) {
                                      _nameControllers[i].text = state.columns[i].columnName; // Reset to columnName
                                    }
                                  },
